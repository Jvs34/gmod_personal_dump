SWEP.SPRITE_SCALE=128.0


SWEP.g_debug_physcannon=0;

SWEP.physcannon_minforce( "physcannon_minforce", "700", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_maxforce( "physcannon_maxforce", "1500", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_maxmass( "physcannon_maxmass", "250", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_tracelength( "physcannon_tracelength", "250", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_chargetime("physcannon_chargetime", "2", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_pullforce( "physcannon_pullforce", "4000", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_cone( "physcannon_cone", "0.97", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.physcannon_ball_cone( "physcannon_ball_cone", "0.997", FCVAR_REPLICATED | FCVAR_CHEAT );
SWEP.player_throwforce( "player_throwforce", "1000", FCVAR_REPLICATED | FCVAR_CHEAT );

if CLIENT then

SWEP.PHYSCANNON_BEAM_SPRITE="sprites/orangelight1.vmt"
SWEP.PHYSCANNON_BEAM_SPRITE_NOZ="sprites/orangelight1_noz.vmt"
SWEP.PHYSCANNON_GLOW_SPRITE="sprites/glow04_noz"
SWEP.PHYSCANNON_ENDCAP_SPRITE="sprites/orangeflare1"
SWEP.PHYSCANNON_CENTER_GLOW="sprites/orangecore1"
SWEP.PHYSCANNON_BLAST_SPRITE="sprites/orangecore2"
...
--Jvs:create this game_shadow_whatever in Lua
...
const float DEFAULT_MAX_ANGULAR = 360.0 * 10.0;
const float REDUCED_CARRY_MASS = 1.0;
...
if !CLIENT then return end
	...
	float flLerpFactor = clamp( flMass, flMaxMass, 500.0 );
...
//	ComputeMaxSpeed( pEntity, pPhys );
	...
	m_errorTime = -1.0; // 1 seconds until error starts accumulating
	m_error = 0;
	m_contactAmount = 0;
...
void CGrabController::DetachEntity( bool bClearVelocity )
{
	...
	if ( pEntity )
	{
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			{
#ifndef CLIENT_DLL
				ClampPhysicsVelocity( pPhys, hl2_normspeed.GetFloat() * 1.5, 2.0 * 360.0 );
#endif
			}
...
IMotionEvent::simresult_e CGrabController::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	...
	{
		m_contactAmount = Approach( 0.1, m_contactAmount, deltaTime*2.0 );
	}
	else
	{
		m_contactAmount = Approach( 1.0, m_contactAmount, deltaTime*2.0 );
	}
...
public:
	...
	void Init( int startAttachment, int endAttachment, CBaseEntity *pEntity, bool firstPerson )
	{
		...
		if ( firstPerson )
		{
			beamInfo.m_flWidth = 0.0;
			beamInfo.m_flEndWidth = 4.0;
		}
		else
		{
			beamInfo.m_flWidth = 0.5;
			beamInfo.m_flEndWidth = 2.0;
		}
	...
	void SetVisible( bool state = true )
	{
		...
		m_pBeam->brightness = ( state ) ? 255.0 : 0.0;
	}
...
void CWeaponPhysCannon::OnDataChanged( DataUpdateType_t type )
{
	...
	if ( m_bOldOpen != m_bOpen )
	{
		if ( m_bOpen )
		{
			m_ElementParameter.InitFromCurrent( 1.0, 0.2, INTERP_SPLINE );
		}
		else
		{	
			m_ElementParameter.InitFromCurrent( 0.0, 0.5, INTERP_SPLINE );
		}
...
void CWeaponPhysCannon::PrimaryFireEffect( void )
{
	...
	UTIL_ScreenFade( pOwner, white, 0.1, 0.0, FFADE_IN );
#endif
...
void CWeaponPhysCannon::PuntNonVPhysics( CBaseEntity *pEntity, const Vector &forward, trace_t &tr )
{
	...
	m_flRepuntObjectTime = gpGlobals->curtime + 0.5;
	...
	m_flElementDebounce = gpGlobals->curtime + 0.5;
...
void CWeaponPhysCannon::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	...
	if ( Pickup_OnAttemptPhysGunPickup( pEntity, pOwner, PUNTED_BY_CANNON ) )
	{
		...
		if ( !Pickup_ShouldPuntUseLaunchForces( pEntity, PHYSGUN_FORCE_PUNTED ) )
		{
			...
			for ( i = 0; i < listCount; i++ )
			{
				const float hitObjectFactor = 0.5;
				const float otherObjectFactor = 1.0 - hitObjectFactor;
				...
				else
				{
  				...
  				pList[i]->ApplyForceCenter( forward * 15000.0 * ratio );
  				pList[i]->ApplyForceOffset( forward * mass * 600.0 * ratio, tr.endpos );
			}
		}
		else
		{
...
#endif
	// Add recoil
	QAngle	recoil = QAngle( random->RandomFloat( 1.0, 2.0 ), random->RandomFloat( -1.0, 1.0 ), 0 );
...
void CWeaponPhysCannon::SecondaryAttack( void )
{
	...
	else
	{
		...
		case OBJECT_BEING_DETACHED:
			m_flNextSecondaryAttack = gpGlobals->curtime + 0.01;
			break;
		}
...
/*	if( pOwner )
	{
		...
		float	loadWeight = ( 1.0 - GetLoadPercentage() );
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject( void )
{
	...
	float	testLength = TraceLength() * 4.0;
	...
	if ( ( tr.fraction != 1.0 ) && ( tr.m_pEnt ) && ( tr.m_pEnt->IsWorld() == false ) )
	{
...
void CWeaponPhysCannon::DoEffectIdle( void )
{
	...
	{
		m_Parameters[i].GetScale().SetAbsolute( random->RandomFloat( 0.075, 0.05f ) * SPRITE_SCALE );
...
float CWeaponPhysCannon::GetLoadPercentage( void )
{
	...
	loadWeight = clamp( loadWeight, 0.0, 1.0 );
	return loadWeight;
}
...
void CWeaponPhysCannon::StartEffects( void )
{
	...
	{
		m_Parameters[PHYSCANNON_CORE].GetScale().Init( 0.0, 1.0, 0.1f );
		m_Parameters[PHYSCANNON_CORE].GetAlpha().Init( 255.0, 255.0, 0.1f );
	...
	{
		m_Parameters[PHYSCANNON_BLAST].GetScale().Init( 0.0, 1.0, 0.1f );
		m_Parameters[PHYSCANNON_BLAST].GetAlpha().Init( 255.0, 255.0, 0.1f );
	...
	for ( int i = PHYSCANNON_GLOW1; i < (PHYSCANNON_GLOW1+NUM_GLOW_SPRITES); i++ )
	{
		...
		m_Parameters[i].GetAlpha().SetAbsolute( 64.0 );
	...
	for ( int i = PHYSCANNON_ENDCAP1; i < (PHYSCANNON_ENDCAP1+NUM_ENDCAP_SPRITES); i++ )
	{
		...
		m_Parameters[i].GetScale().SetAbsolute( 0.05f * SPRITE_SCALE );
		m_Parameters[i].GetAlpha().SetAbsolute( 255.0 );
...
void CWeaponPhysCannon::DoEffectReady( void )
{
	...
	if ( IsCarriedByLocalPlayer() && !::input->CAM_IsThirdPerson())
	{
		...
		m_Parameters[PHYSCANNON_CORE].GetScale().InitFromCurrent( 14.0, 0.2f );
		m_Parameters[PHYSCANNON_CORE].GetAlpha().InitFromCurrent( 128.0, 0.2f );
		m_Parameters[PHYSCANNON_CORE].SetVisible();
	}
	else
	{
		//Turn off the center sprite
		m_Parameters[PHYSCANNON_CORE].GetScale().InitFromCurrent( 8.0, 0.2f );
		m_Parameters[PHYSCANNON_CORE].GetAlpha().InitFromCurrent( 0.0, 0.2f );
		m_Parameters[PHYSCANNON_CORE].SetVisible();
	}
	...
	for ( int i = PHYSCANNON_GLOW1; i < (PHYSCANNON_GLOW1+NUM_GLOW_SPRITES); i++ )
	{
		...
		m_Parameters[i].GetAlpha().InitFromCurrent( 64.0, 0.2f );
...
void CWeaponPhysCannon::DoEffectHolding( void )
{
	...
	if ( IsCarriedByLocalPlayer() && !::input->CAM_IsThirdPerson() )
	{
		...
		m_Parameters[PHYSCANNON_CORE].GetScale().InitFromCurrent( 16.0, 0.2f );
		m_Parameters[PHYSCANNON_CORE].GetAlpha().InitFromCurrent( 255.0, 0.1f );
		m_Parameters[PHYSCANNON_CORE].SetVisible();
...
void CWeaponPhysCannon::DoEffectLaunch( Vector *pos )
{
	...
	m_Parameters[PHYSCANNON_BLAST].GetScale().Init( 8.0, 64.0, 0.1f );
	m_Parameters[PHYSCANNON_BLAST].GetAlpha().Init( 255.0, 0.0, 0.2f );
