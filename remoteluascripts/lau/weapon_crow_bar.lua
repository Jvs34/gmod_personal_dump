...
SWEP.CrowMaxStamina=10			--4 seconds of fly time
...
function SWEP:Initialize()
	...
	self.dt.NextSound=CurTime()+1
end

function SWEP:SetupDataTables()
	self:DTVar( "Float", 0, "AttackTime")
	self:DTVar( "Float", 3, "NextSound")
...
function SWEP:Reload()
	if self.dt.NextSound > CurTime() then return end
	self:EmitSound("NPC_Crow.Alert")
	self.dt.NextSound=CurTime()+2
end
...
function SWEP:Think()
	...
	elseif (not ply:KeyDown(IN_ATTACK2) or ply:OnGround() or ply:WaterLevel()>1 or not self:CanFly()) then
		if self.dt.Flying then
			self.dt.AttackTime=CurTime()+self.AttackTime
			self.dt.NextStaminaDrain=CurTime()-0.1
			self:DrainStamina()	--drain stamina again to prevent fly tapping abuse
		end
		self.FlapSound:Stop()
		self.dt.Flying=false
	end
