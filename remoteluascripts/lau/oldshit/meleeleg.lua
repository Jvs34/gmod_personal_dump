...
SWEP.DrawAmmo= true
SWEP.PrintName= "Melee Leg"
SWEP.Author= "Jvs"
SWEP.DrawCrosshair= true
SWEP.ViewModelFOV= 54
SWEP.RenderGroup = RENDERGROUP_BOTH
SWEP.CSM=nil;
SWEP.IsMeleeLeg=true;
...
SWEP.WeaponModel={
	...
	OffsetVector=Vector(0,3,5),
	OffsetAngle=Vector(180,0,0),
	BoneMerge=(EF_BONEMERGE|EF_BONEMERGE_FASTCULL|EF_PARENT_ANIMATES),
	v={Vector(0,2,0),Vector(0,-1,0)},
}
...
hook.Add("PreDrawViewModel", "MeleelegDrawPre", function(vm, pl, weapon)
...
local vm = self.Owner;
local matrix = vm:GetBoneMatrix(vm:LookupBone("ValveBiped.Bip01_R_Thigh"))
...
local BB=function(self)
			local bonename="ValveBiped.Bip01_R_Thigh"
			local bm = self:GetBoneMatrix( self:LookupBone(bonename) )
			if !bm then return end
			if IsValid(self:GetActiveWeapon()) && self:GetActiveWeapon().IsMeleeLeg then
...
function SWEP:CreateClientSideArm()
	timer.Simple(0,function()
		...
		if self.Owner.BuildBonePositions then
			...
			self.Owner.BuildBonePositions=function(...) oldbb(...) BB(...) end
		else
		self.Owner.BuildBonePositions=BB;
		end
		end
		...
		self.MeleeHand.BuildBonePositions=function(self)
			...
			for i=0,self:GetBoneCount()-1 do
				...
				if !bm || (string.find(string.lower(bonename),"thigh") || string.find(string.lower(bonename),"calf") || string.find(string.lower(bonename),"foot") || string.find(string.lower(bonename),"toe")) && !string.find(string.lower(bonename),"bip01_l_") then continue end
				bm:Scale( vector_origin )
				self:SetBoneMatrix(self:LookupBone(bonename), bm )
			end
...
function SWEP:PrimaryAttack()
    ...
    if (tr.Hit)then
        if !tr.HitWorld && IsValid(tr.Entity) && SERVER then
            self.Owner:TraceHullAttack( self.Owner:GetShootPos(), tr.HitPos, Vector( -16, -16, -16 ), Vector( 36, 36, 36 ), 60, DMG_SLASH, 0.75 );
        end
...
function SWEP:Think()
 ...
 local function MeleeLegImpairement(ply, data)
	if IsValid(ply) && IsValid(ply:GetActiveWeapon()) && ply:GetActiveWeapon().IsMeleeLeg then
...
hook.Add("Move", "MeleeLegImpairement", MeleeLegImpairement)
weapons.Register(SWEP,"weapon_melee_leg",true)