...
if CLIENT then
    ...
    hook.Add("RenderScreenspaceEffects","JintoGlow",function( )
        ...
        MaterialBlurX:SetFloat( "$size", 6 )
        MaterialBlurY:SetFloat( "$size", 6 )
	...
	function SEFFECT:Init(data)
		...
		self.CP:SetModelScale(self.mSize,0)
		self.CP:Spawn()
		
		self.Beam=ClientsideModel(self.BeamModel)
		self.Beam:SetModelScale(self.mSize,0)
	...
	concommand.Add("pingtool", function(ply,command,args)
		...
		local tr=PlayerTrace(ply,ply:GetAimVector(),MarkersTypes[mType].ignoreEntities)
	...
	local function HandleMarker(ply,mType,mEnt,mPos,mSize)
		...
		if mPos then
			local tr_mNormal=(mPos-ply:EyePos()):GetNormal()
		...
		umsg.Start("PingTool",rf)
			...
			if IsValid(mEnt) then umsg.Entity(mEnt) end
			umsg.Bool(mPos~=nil)
			if mPos then 
