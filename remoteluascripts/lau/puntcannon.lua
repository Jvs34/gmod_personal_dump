...
function SWEP:SetupDataTables()
	...
	self:NetworkVar( "Bool", 2, "Repulse")
...
function SWEP:Think()
    ...
    if self.dt.IsCharged and self.dt.NextCharge < CurTime() then
		...
		self.ChargeSound:ChangePitch(100+self.dt.Charge/1.7,0)
...
hook.Add("Move","Puntcannonmove",function(ply,data)
	if IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon().IsPuntCannon and ply:GetActiveWeapon().dt  then
		if ply:GetActiveWeapon():GetRepulse() then
			ply:SetGroundEntity(NULL)
			--
			data:SetVelocity(ply:GetVelocity()+ply:GetAimVector()*-4*(100))
			ply:GetActiveWeapon():SetRepulse(false)
			
			return
		end
		if ply:GetActiveWeapon().dt.Attached then
