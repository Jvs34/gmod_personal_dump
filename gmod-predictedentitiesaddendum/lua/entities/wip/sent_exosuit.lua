...
function ENT:SetupDataTables()
	...
	self:DefineNWVar("String","OwnerModel")
	self:DefineNWVar("Vector","DTVelocity")
	self:DefineNWVar("Entity","DTGroundEntity")
	self:DefineNWVar("Angle","DTEyeAngles")
end
...
if SERVER then
	...
	function ENT:DoInitPhysics()
		...
		if IsValid(self:GetPhysicsObject())then
			...
			self:GetPhysicsObject():SetMaterial("material")
		end
	end
	...
	function ENT:Draw( flags )
		...
		--we usually draw it
		local drawlp = self.DrawLocalPlayer
...
function ENT:HandleAnimations()
	...
	--REMOVE ME
	self.Skeleton:ResetSequence( self.Skeleton:LookupSequence( "run_dual" ) )
	...
	self.Skeleton:SetPoseParameter( "move_x", 1) 
