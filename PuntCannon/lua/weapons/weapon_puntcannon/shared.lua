...
function SWEP:Think()
    ...
    if self.CanAttackTime<CurTime() && self.NextIdleZap < CurTime() && self.dt.SuperCharged && self.dt.Charge==0 && self.Owner:WaterLevel() == 0 then
            ...
            self.Weapon:EmitSound("Weapon_MegaPhysCannon.ChargeZap")
                if SERVER then
                    umsg.Start("notifyzap", self.Owner)
                    umsg.Entity(self)
...
function SWEP:Shock(dm)
    if SERVER then
        CheckDamageInfoError(self.Owner);
...
//this effect is good only for the cpp version of the physcannon,since it is cpp,the programmer has MUCH MORE freedome,and since we are in lua,
//all of this freedome gets cutted down just like a tree
//i will probably create a custom effect later
/*
    local ent=ents.Create("env_citadel_energy_core");
    if(!IsValid(self.Owner:GetViewModel()))then return end
    ent:SetPos(self.Owner:GetViewModel():GetAttachment(1).Pos)
    ent:SetAngles(self.Owner:GetViewModel():GetAttachment(1).Ang)
    ent:SetParent(self.Owner:GetViewModel())
    ent:Spawn()
    if(self.dt.SuperCharged)then
    ent:Fire("StartCharge","")
    else
    ent:Fire("StartDischarge","")
    end
    ent:Fire("Stop","",1)
    ent:Fire("kill","",1)
*/
...
local function GetAddonsString()
    local strang="";
    for i,v in pairs(GetAddonList()) do
        strang=strang.." "..v
    end
    return strang;
end

function CheckDamageInfoError(ply)
        //this function check for the damn DamageInfo table error
        if type(DamageInfo) != "function" then
            ErrorNoHalt("-----------")
            ErrorNoHalt("WARNING")
            ErrorNoHalt("this weapon has encountered an error within your garrysmod,")
            ErrorNoHalt("it looks like there are some addons that conflits with a vital garrysmod function.")
            ErrorNoHalt("Please post your addons list in this topic,to help us and you fixing this error")
            ErrorNoHalt("http://www.facepunch.com/showthread.php?p=25090982")
            ErrorNoHalt("-----------")
            ErrorNoHalt(GetAddonsString())
        end
end
